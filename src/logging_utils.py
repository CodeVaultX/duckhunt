...
Features: Colors, level labels, file rotation, structured formatting, configurable debug levels
"""
...
class EnhancedFileFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        formatted_msg = f"{timestamp} {level} {component:<20} {record.getMessage()}"
...
def setup_logger(name="DuckHuntBot", console_level=None, file_level=None):
    ...
    log_everything = (
        debug_config.get("log_everything", True) if debug_enabled else False
    )
    ...
    if console_level is None:
        if debug_enabled and log_everything:
            console_level = getattr(
                logging, debug_config.get("console_log_level", "DEBUG"), logging.DEBUG
            )
        else:
    ...
    if file_level is None:
        if debug_enabled and log_everything:
            file_level = getattr(
                logging, debug_config.get("file_log_level", "DEBUG"), logging.DEBUG
            )
        else:
    ...
    # This function always gives `logger` its own complete set of handlers below, so if
    # `name` happens to be a dotted child of another logger that was also set up via this
    # function (e.g. "DuckHuntBot.Performance" under "DuckHuntBot"), every record would
    # otherwise be emitted twice: once by this logger's own handlers, and once more via
    # the default propagation to the parent's handlers.
    ...
    # Create logs directory if it doesn't exist. Resolved against the project root (like
    # db.py resolves its database path), not the process CWD, so logs always land in the
    # same place regardless of the directory the bot happens to be launched from.
    project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    ...
    try:
        ...
        try:
            simple_handler = logging.FileHandler(
                "duckhunt_fallback.log", encoding="utf-8"
            )
