...
def with_retry(config: Optional[RetryConfig] = None, exceptions: tuple = (Exception,)):
...
class ErrorRecovery:
    ...
    @staticmethod
    def validate_input(
        value: Any, validator: Callable, default: Any = None, field_name: str = "input"
    ) -> Any:
        ...
        for name, check in self.checks.items():
            try:
                ...
                if check["critical"] and check["failure_count"] >= 3:
                    self.logger.error(
                        f"Critical health check '{name}' failed {check['failure_count']} times: {e}"
                    )
...
def safe_format_message(template: str, **kwargs) -> str:
    ...
    try:
        ...
        def replace_missing(match):
            field = match.group(1)
            # A format field can be "name", "name!r", "name:>10", or "name!r:>10" -
            # only the part before "!" or ":" is the actual keyword argument name.
            # Previously the whole field (including the format spec) was checked
            # against safe_kwargs, so any field with a spec like "{value:>10}" was
            # always treated as "missing" even when "value" was present.
            var_name = field.split("!")[0].split(":")[0]
...
def sanitize_user_input(
    value: str, max_length: int = 100, allowed_chars: Optional[str] = None
) -> str:
