...
class LevelManager:
        ...
        try:
            if os.path.exists(self.levels_file):
                with open(self.levels_file, "r", encoding="utf-8") as f:
                    ...
                    self.logger.info(
                        f"Loaded {level_count} levels from {self.levels_file}"
                    )
            else:
                # Fallback levels if file doesn't exist
                self.levels_data = self._get_default_levels()
                self.logger.warning(
                    f"{self.levels_file} not found, using default levels"
                )
    ...
    def calculate_player_level(self, player: Dict[str, Any]) -> int:
        ...
        if method == "xp":
            ...
            total_ducks = player.get("ducks_shot", 0) + player.get(
                "ducks_befriended", 0
            )
    ...
    def get_player_level_info(self, player: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if method == "xp":
            ...
            current_value = player.get("ducks_shot", 0) + player.get(
                "ducks_befriended", 0
            )
            value_type = "ducks"
        ...
        return {
            ...
            "duck_spawn_speed_modifier": level_data.get(
                "duck_spawn_speed_modifier", 1.0
            ),
            "magazines": level_data.get("magazines", 3),
            "bullets_per_magazine": level_data.get("bullets_per_magazine", 6),
            "current_xp": player.get("xp", 0),
            "total_ducks": player.get("ducks_shot", 0)
            + player.get("ducks_befriended", 0),
    ...
    def get_modified_accuracy(self, player: Dict[str, Any]) -> int:
        ...
        base_accuracy = player.get(
            "accuracy", 75
        )  # This will be updated by bot config in create_player
    ...
    def get_modified_befriend_rate(
        self, player: Dict[str, Any], base_rate: float = 75.0
    ) -> float:
    ...
    def update_player_magazines(
        self, player: Dict[str, Any], full_reload: bool = False
    ) -> Dict[str, Any]:
        """Update player's magazine count based on their current level.

        If `full_reload` is True, skip carrying over existing ammo and just give the
        player a full magazine plus all spare magazines at their level's capacity. This
        is for rearm-style flows (admin `!rearm`, auto-rearm-on-duck-shot) that are meant
        to fully restore the player's loadout, as opposed to a level-up recalculation
        (the default/prorated behavior below), which must never grant free ammo by
        rounding a partial carry-over up to a full magazine.
        """
        level_info = self.get_player_level_info(player)
        ...
        total_current_bullets = (
            current_ammo + max(0, current_magazines - 1) * current_bullets_per_mag
        )
        ...
        if full_reload:
            ...
            return {
                ...
                "new_total_bullets": level_magazines * level_bullets_per_mag,
        ...
        # Calculate how to distribute bullets. `new_magazines` is the TOTAL magazine count
        # (including the currently-loaded one), derived from how many bullets the player
        # actually carries over - it must never be rounded UP past that, or leveling up
        # would silently grant free ammo in the form of extra full "spare" magazines the
        # player never earned.
        if new_total_bullets <= 0:
            player["current_ammo"] = 0
            new_magazines = 1
        elif new_total_bullets <= level_bullets_per_mag:
            # All bullets fit in current magazine; no full spare magazines earned
            player["current_ammo"] = new_total_bullets
            new_magazines = 1
        else:
            # Fill current magazine, then only count *full* spare magazines from the
            # remainder (floor division - never round partial magazines up to a full one)
            player["current_ammo"] = level_bullets_per_mag
            remaining_after_current = new_total_bullets - level_bullets_per_mag
            spare_full_magazines = remaining_after_current // level_bullets_per_mag
            new_magazines = 1 + spare_full_magazines

        new_magazines = max(1, min(new_magazines, level_magazines))
